...
import { Spinner, ErrorBox, Badge, SectionHeader } from "./ui";
...
function resolveRiskLevel(row) {
  return row?.risk_level ?? row?.riskLevel ?? row?.level ?? row?.status ?? "LOW";
}

function getRiskBand(riskLevel) {
  const level = String(riskLevel || "").toLowerCase();

  if (level.includes("high") || level.includes("red")) {
    return "high";
  }

  if (level.includes("medium") || level.includes("yellow") || level.includes("mid")) {
    return "medium";
  }

  if (level.includes("low") || level.includes("green")) {
    return "low";
  }
...
const RISK_GLOW_COLORS = {
  high: "var(--glow-high)",
  medium: "var(--glow-medium)",
  low: "var(--glow-low)",
  unknown: "var(--glow-info)",
};

function RiskScoreMeter({ score, riskLevel }) {
    ...
    <div className="conf-meter">
      <div className="conf-bar-bg">
        <div
          ...
          style={{
            width: `${pct}%`,
            background: RISK_BAR_COLORS[band],
            boxShadow: `0 0 6px ${RISK_GLOW_COLORS[band]}`,
          }}
        />
      </div>
      <span
        className="conf-label"
        style={{ color: RISK_LABEL_COLORS[band], fontWeight: 600 }}
      >
        {pct}%
      </span>
    </div>
...
export default function RiskHeatmap({ uploadResult }) {
  ...
  const hasData = singleRisk || heatmapData;

  return (
    <div className="panel">
      <SectionHeader
        icon="🌡"
        title="Denial Risk Queue"
      ...
      {singleRisk && (
        <div className="single-risk-card">
          <div className="field-row">
            ...
            <RiskScoreMeter
              score={singleRisk.risk_score}
              riskLevel={resolveRiskLevel(singleRisk)}
            />
          </div>
          <div className="field-row">
        ...
        <>
          ...
          <div className="heatmap-table-wrap">
            <table className="heatmap-table">
              ...
              <tbody>
                {results.map((r, i) => (
                  <tr key={i} className={`row-${(resolveRiskLevel(r) ?? "LOW").toLowerCase()}`}>
                    <td>{i + 1}</td>
                    <td>{r.payer ?? "N/A"}</td>
                    <td>
                      {Array.isArray(r.cpt_codes) ? r.cpt_codes.join(", ") : r.cpt_codes ?? "N/A"}
                    </td>
                    <td>
                      <RiskScoreMeter score={r.risk_score} riskLevel={resolveRiskLevel(r)} />
                    </td>
                    <td>
                      <Badge level={resolveRiskLevel(r)} />
                    </td>
      ...
      {!hasData && !loading && (
        <p className="hint">
          Score the demo queue to preview risk distribution, or score your uploaded claim.
        </p>
      )}
    </div>
  );
