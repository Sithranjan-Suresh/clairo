...
export function ConfidenceMeter({ score }) {
  ...
  const band = pct >= 70 ? "high" : pct >= 40 ? "mid" : pct >= 1 ? "low" : "info";
  return (
    <div className="conf-meter" data-score={band}>
...
export function SectionHeader({ icon, title, subtitle }) {
  return (
    <div className="section-header">
      ...
      <div>
        <h2 className="section-title font-ui">{title}</h2>
        {subtitle && <p className="section-sub font-body">{subtitle}</p>}
      </div>
    </div>
  );
}
